...
# 1. Your S3 bucket name (from AWS S3 setup)
S3_BUCKET_NAME = 'my-image-label-repo-2025' # e.g., 'my-image-label-repo-2025'

# 2. Your AWS region (from AWS S3 and IAM setup)
AWS_REGION = 'us-east-1' # e.g., 'ap-southeast-1'

# 3. The S3 key (path/filename) of the image you want to analyze
IMAGE_S3_KEY = 'images/my_test_image.jpg' # e.g., 'my_test_image.jpg' or 'images/my_test_image.jpg'

# Initialize AWS clients
# These clients use the credentials configured via 'aws configure' (Step 5)
s3_client = boto3.client('s3', region_name=AWS_REGION)
rekognition_client = boto3.client('rekognition', region_name=AWS_REGION)

# --- Function to detect labels using AWS Rekognition (Implements Step 3) ---
...
# --- Function to process and display labels using Matplotlib (Implements Step 7 & 8) ---
def process_and_display_labels_matplotlib(image_s3_key, rekognition_response):
    ...
    # 1. Download image from S3 into memory (Part of Step 2: Extract the image)
    try:
    ...
    # 2. Prepare plot for visualization (Step 8)
    fig, ax = plt.subplots(1)
    ax.imshow(img) # Display the image

    # 3. Extract and display labels and bounding boxes (Step 7 & 8)
    print("\n--- Detected Labels ---")
    ...
    # 4. Finalize and show plot (Step 8)
    ax.axis('off') # Hide axes for cleaner image display
    plt.title(f"Labels for: {os.path.basename(image_s3_key)}")
    plt.show() # Display the plot window

# --- Main execution logic (Orchestrates the workflow - Part of Step 6) ---
if __name__ == "__main__":
    print("Starting Image Label Generator workflow...")

    # 1. Call Rekognition to detect labels (incorporating Step 3)
    rekognition_response = detect_labels_from_s3(IMAGE_S3_KEY)

    # 2. Process and display the results (incorporating Step 7 & 8 with Matplotlib)
